...
suite("Extension Test Suite", () => {
  ...
  test("Find multiple Tooltips By Keyword", () => {
    const mockTooltips = [
      { keyword: "test", tooltip: "This is a test tooltip" },
      { keyword: "test", tooltip: "This is an example tooltip" },
    ];

    const matchedTooltips = findTooltipsByKeyword("test", mockTooltips);
    assert(matchedTooltips.length === 2);
    assert(matchedTooltips[1].tooltip === "This is an example tooltip");
  });
});
